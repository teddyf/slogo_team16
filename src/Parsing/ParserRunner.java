...
public class ParserRunner {
    ...
    public ParserRunner (String language, ProgramParser parser) {
        ...
        parser.setSymbols(PATH + language);
        parser.setSyntax(PATH_SYNTAX);
    }
    ...
    private String[] parseLine (ProgramParser lang, String input) {
        ...
        for (int i = 0; i < text.length; i++) {
            String s = text[i];
            if (s.trim().length() > 0) {
                System.out.println(String.format("%s : %s : %s", s, lang.getSymbol(s),
                                                 lang.getSymbolType(s)));
                if(isNumber(s)){
                    names.add(s);
                }
                else names.add(lang.getSymbol(s));
                symbols.add(lang.getSymbolType(s));
            }
        }
    ...
    private boolean isMethod (String input) {
        return parser.getSymbolType(input).equals("Command");
    }
